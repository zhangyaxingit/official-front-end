<template>
  <div class="news-container">
    ...
    <!-- <div>{{curNews}}</div> -->
    <div class="base-info">
      <h3>{{curNews.title}} <el-button
          type="text"
          @click="showBannerDialog"
          style="margin-left: 10px; padding-right: 8px; padding-left: 8px"
          >编辑基本信息</el-button></h3>
      <p class="summary">概述： {{curNews.desc || '--'}}</p>
      <p class="news-type">所属类目： {{curNews.type}}</p>
    </div>
    <el-divider></el-divider>
    <div id="ArticleTextContent">
      <TextEditor :txt="curNews.html" @change="htmlChange"/>
    </div>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="eidtNews">保存</el-button>
        </span>
      </template>
    </el-dialog>
...
export default defineComponent({
  ...
  setup() {
    ...
    const list = computed(() => store.state.news.list)
    ...
    onMounted(() => {
      ...
      opType.value = `${type}`
      curNews.value = news.value.list.find((item:any) => item.id == id)
      // console.log(news.value)
      // console.log(news.value.find((item:any) => item.id == id).html)
    })
    ...
    const showBannerDialog = () => {
      ...
      NewsForm.value = {
        title: curNews.value.title,
        desc: curNews.value.desc,
        type: parseInt(curNews.value.type, 10) ,
      }
    }
    ...
    const eidtNews = () => {
      ...
      const newData = {
          ...
          status: curNews.value.status,
          html: curNews.value.html,
          id: curNews.value.id,
        }
      store.dispatch("news/updateNews", {
        data: newData,
        cb: () => {
          curNews.value = newData
        }
      });
    }
