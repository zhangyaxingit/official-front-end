    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="eidtSolve">保存</el-button>
        </span>
      </template>
    </el-dialog>
...
export default defineComponent({
  ...
  setup() {
    ...
    const list = computed(() => store.state.solve.list)
    ...
    onMounted(() => {
      ...
      opType.value = `${type}`
      curSolve.value = solve.value.list.find((item:any) => item.id == id)
      // console.log(solve.value)
      // console.log(solve.value.find((item:any) => item.id == id).html)
    })
    ...
    const showBannerDialog = () => {
      ...
      solveForm.value = {
        title: curSolve.value.title,
        desc: curSolve.value.desc,
      }
    }
    ...
    const eidtSolve = () => {
      ...
      const newData = {
          ...
          status: curSolve.value.status,
          html: curSolve.value.html,
          id: curSolve.value.id,
        }
      store.dispatch("solve/updateSolve", {
        data: newData,
        cb: () => {
          curSolve.value = newData
        }
      });
    }
